export default function ProjectCard({ project }) {
  const { title, description, stack = [], live, repo } = project;
  ...
  return (
    <article>
      ...
      <div className="card-links">
        {live && (
          <a href={live} target="_blank" rel="noreferrer" aria-label={`${title} live demo`}>
            Live ↗
          </a>
        )}

        {repo && (
          <a href={repo} target="_blank" rel="noreferrer" aria-label={`${title} GitHub repository`}>
            {/* tiny GitHub mark + label */}
            <svg
              aria-hidden="true"
              width="16" height="16" viewBox="0 0 16 16" style={{ marginRight: 6, verticalAlign: "-2px" }}
            >
